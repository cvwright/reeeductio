...
class TestWebSocketManager:
    ...
    @pytest.mark.asyncio
    async def test_connect_new_channel(self, manager, mock_websocket):
        """Test connecting to a new channel"""
        channel_id = "test_channel_123"

        await manager.connect(channel_id, mock_websocket)

        mock_websocket.accept.assert_called_once()
        assert channel_id in manager.active_connections
        assert mock_websocket in manager.active_connections[channel_id]

    @pytest.mark.asyncio
    async def test_connect_existing_channel(self, manager, mock_websocket):
        """Test connecting multiple clients to the same channel"""
        channel_id = "test_channel_123"
        ws2 = AsyncMock(spec=WebSocket)
        ws2.accept = AsyncMock()

        await manager.connect(channel_id, mock_websocket)
        await manager.connect(channel_id, ws2)

        assert len(manager.active_connections[channel_id]) == 2
        assert mock_websocket in manager.active_connections[channel_id]
        assert ws2 in manager.active_connections[channel_id]

    def test_disconnect_removes_connection(self, manager, mock_websocket):
        """Test disconnecting removes the connection"""
        channel_id = "test_channel_123"
        manager.active_connections[channel_id] = {mock_websocket}

        manager.disconnect(channel_id, mock_websocket)

        assert channel_id not in manager.active_connections
    ...
    def test_disconnect_keeps_other_connections(self, manager, mock_websocket):
        ...
        channel_id = "test_channel_123"
        ws2 = AsyncMock(spec=WebSocket)
        manager.active_connections[channel_id] = {mock_websocket, ws2}

        manager.disconnect(channel_id, mock_websocket)

        assert channel_id in manager.active_connections
        assert mock_websocket not in manager.active_connections[channel_id]
        assert ws2 in manager.active_connections[channel_id]

    def test_disconnect_nonexistent_channel(self, manager, mock_websocket):
        """Test disconnecting from non-existent channel doesn't error"""
        manager.disconnect("nonexistent_channel", mock_websocket)
    ...
    async def test_broadcast_to_nonexistent_channel(self, manager):
        """Test broadcasting to non-existent channel doesn't error"""
        message = {"type": "message", "content": "Hello"}
        await manager.broadcast_message("nonexistent_channel", message)
        # Should not raise an error

    @pytest.mark.asyncio
    async def test_broadcast_removes_dead_connections(self, manager, mock_websocket):
        """Test that failed broadcasts remove dead connections"""
        channel_id = "test_channel_123"
        ...
        manager.active_connections[channel_id] = {ws_dead, ws_alive}

        message = {"type": "message", "content": "Hello"}
        await manager.broadcast_message(channel_id, message)

        # Dead connection should be removed
        assert ws_dead not in manager.active_connections[channel_id]
        # Alive connection should remain
        assert ws_alive in manager.active_connections[channel_id]
